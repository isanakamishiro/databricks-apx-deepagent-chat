...
- Databricks Appsが有効なDatabricks Workspace
...
| AI基盤 | Databricks Apps + LangChain + deepagents |
...
チャット画面で指定したUCボリュームのルートに以下のファイル・ディレクトリを置くことで、エージェントの動作をカスタマイズできます。

### エージェントメモリ

ボリュームのルートに `AGENTS.md` を置くと、その内容が起動時にエージェントの永続メモリとして読み込まれます。

```
<volume>/
└── AGENTS.md   # エージェントメモリ — 指示・コンテキスト・ペルソナなど
```

### カスタムスキル

ボリュームのルート直下に `skills/` ディレクトリを作成し、スキルファイルを置くと自動的に読み込まれ、エージェントから利用可能になります。
...
アプリケーションコードを変更せずにエージェントの機能を拡張できます。
...
- Databricks Workspace（Databricks Appsが有効）
